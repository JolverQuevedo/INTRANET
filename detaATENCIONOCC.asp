...
<%DO WHILE NOT RS.EOF%>
    <tr  bgcolor="<% if CONT mod 2  = 0 THEN 
        ...
        <td class="DESCRIPTORES" width="15%" style="text-align:right; padding-right:10px;"><%=FORMATNUMBER(CDBL(trim(RS.FIELDS.ITEM(4)))-CDBL(trim(RS.FIELDS.ITEM(5)))+CDBL(trim(RS.FIELDS.ITEM(5))),3,,,TRUE)%></td>
...
<div class="parte">
    ...
    <%'**************************%>
        ...
        <tr bgcolor='<%=Application("color1")%>'>
            ...
            <td colspan="3"  class="TALASBLUE22" style="text-align:left"> Incluida la tolerancia definida como par�metro</td>
...
function vale() {
   ...
   if (isNaN(aten)) {
    ...
    if (parseFloat(aten) > parseFloat(xrec)) {
        alert("No puede recibir m�s de lo pendiente")
...
function graba(opc) {
  ...
  //   alert(cad)
    ...
    window.document.all.body0.src = 'comun/inseratenocc.asp' + cad
 }
