   ...
   rs.open cad, cnn
    ...
    if rs.recordcount >0 then
            ...
            ' busca si tiene codigo en el DBPROD
             ...
             ' response.write (cad)
              ...
              ' TIENE CODIGO DEL DBPROD, ENTONCES SOLO ACTUALIZO EN EL MODELADOR
                ...
                ' NO TIENE CODIGO EN EL DBPROD, ENTONCES CREO UN NUEVO C�DIGO 
                rs.close
              rs.open "select corr_pda,letra_estilo from tb_ctrl", cnn
        ...
        if nume > 9998 then
                    ...
                    For i = 0 To 23
                        if ultimo=nuevaserie(i) then 
                            s=i
                        end if
                    Next
                    ultimo=nuevaserie(s+1)
                else
                    nume=right("0000"&nume,4)
                end if
                response.Write(ultimo&""&nume)
                cad = "update tb_ctrl set corr_pda = "&nume&",letra_estilo='"&ultimo&"'"

                ' ACTUALIZA CORRELATIVO DE CODIGOS
                cnn.execute cad
                codigo = trim(codigo) + trim(cstr(ultimo&""&nume))
...
function BUSCA(url, alias) {
    if (trim(window.thisForm.kod.value) == '' && trim(window.thisForm.Des.value)=='') {
        alert("Debe informar el C�digo de Estilo o Codigo Modelador");
