...
WHILE EXISTS(select * from #tmp4 where REPLACE(COMBO,' ' , '') =  @com)
	BEGIN
...
-- esto es lo que hay en la explosion de telas, hay que poner las cantidades de la po_partida
	...
	where REPLACE(t5.COMBO,' ' , '') =  @com and t5.chk = '1' 
	and t5.po not in (Select (po) from #tmp5 as d where  REPLACE(COMBO,' ' , '') =  @com and chk='')
	order by po
