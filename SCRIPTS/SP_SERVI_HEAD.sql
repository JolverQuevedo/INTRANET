...
ALTER PROCEDURE dbo.SP_SERVI_HEAD @OS CHAR(9)
AS
IF @OS = 'AUTO'
...
'' AS USUARIO, GETDATE() AS FECENT, 'CALLE CAPITAN SALVADOR CARMONA 280 - ATE' AS ENTREGA, '' AS GLOSA, 0 as cant
ELSE
...
(SELECT TG_CDESCRI FROM RSFACCAR..AL0001TABL WHERE TG_CCOD = '31' AND SITORD COLLATE Modern_Spanish_CI_AS = TG_CCLAVE COLLATE Modern_Spanish_CI_AS) AS EDO, 
TIPSER, DESCRIPCION AS DESTIP,  AC_CDIRECC AS DIREC,  AC_CTELEF1,  impMN, impUS, SS.USUARIO, FECENT, ENTREGA, GLOSA,
(select sum(cant) from servidet where numord = @os) as cant
